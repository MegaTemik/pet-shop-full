...
func TestGetOrderItemsByOrderID_Success(t *testing.T) {
	GetOrderItemsByOrderIDMock := mocks.NewOrders(t)
	...
	id := 1
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/orders/%d/items", id), nil)
	w := httptest.NewRecorder()

	handler := New(slog.Default(), GetOrderItemsByOrderIDMock)

	r.Get("/orders/{id}/items", handler.GetOrderItemsByOrderID)
	r.ServeHTTP(w, req)
...
func TestGetOrderItemsByOrderID_Fail(t *testing.T) {
	...
	GetOrderItemsByOrderIDMock.
		On("GetOrderItemsByOrderID", mock.Anything, mock.Anything).
		Return([]models.OrderItem{}, errors.New("server error")).
		Once()
...
func TestGetOrderItemsByOrderID_BadRequest(t *testing.T) {
	...
	id := "not-a-number"
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/orders/%s/items", id), nil)
	w := httptest.NewRecorder()
...
func TestPlaceOrder_Success(t *testing.T) {
	...
	handler := New(slog.Default(), PlaceOrderMock)
