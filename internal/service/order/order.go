...
func (os *OrderService) GetUserOrderHistory(ctx context.Context, email string) ([]models.OrderDetail, error) {

	history, err := os.storage.GetUserOrderHistory(ctx, email)
	if err != nil {
		if errors.Is(err, storage.ErrNotFound) {
